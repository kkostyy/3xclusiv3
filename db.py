...
async def migrate_db():
    ...
    async with aiosqlite.connect(_p()) as db:
        ...
        # New product columns for gender/used
        for col in ["gender TEXT DEFAULT 'unisex'", "photo_ids TEXT DEFAULT NULL", "stock_qty INTEGER DEFAULT 1", "is_used INTEGER DEFAULT 0"]:
            try:
...
async def add_product(name, price, cat, desc, photo_id, gender='unisex') -> int:
    async with aiosqlite.connect(_p()) as db:
        ...
        for col in ["gender TEXT DEFAULT 'unisex'", "photo_ids TEXT DEFAULT NULL", "stock_qty INTEGER DEFAULT 1", "is_used INTEGER DEFAULT 0"]:
            try:
                ...
                pass
        is_used = 1 if cat == 'used' else 0
        c = await db.execute("INSERT INTO products(name,price,category,description,photo_id,gender,is_used) VALUES(?,?,?,?,?,?,?)",
                             (name, price, cat, desc, photo_id, gender, is_used))
