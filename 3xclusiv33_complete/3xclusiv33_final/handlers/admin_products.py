...
from keyboards import kb_admin_products, kb_product_list, kb_cat_admin, kb_menu, kb_cancel, kb_stock_list
...
async def ap_photo(message: Message, state: FSMContext):
    ...
    pid   = await add_product(data["p_name"], data["p_price"], data["p_cat"], data["p_desc"], photo)
    await state.clear()
